...
| 变量 | 默认 | 说明 |
|---|---|---|
| `APP_HOST` | `0.0.0.0` | 应用监听地址。`:3000` 可直连；设 `127.0.0.1` 则只有 nginx 能连 |
| `APP_PORT` | `3000` | 应用端口 |
| `MANAGE_NGINX` | `1` | 设 `0` 则只管应用，不碰 nginx |

健康检查始终连 `127.0.0.1`——`0.0.0.0` 是绑定地址，不是可连接的目标地址。

⚠️ 默认绑 `0.0.0.0` 意味着 `:3000` 绕过了 nginx，所以从那个端口访问**没有 Range
支持**，音频 seek 会退化。对外提供服务请走 80 端口。阿里云安全组不放开 3000 的
话，公网访问不到，只有 VPC 内可达。
